...
def test_prepare_key_shape_is_the_same_with_no_document_versions(app_harness: AppHarness, mint_token):
    response = _call(
        app_harness,
        mint_token(sub=SHARED_SUB),
        "solstice_prepare_operation_version",
...
def test_commit_rejects_unknown_type(app_harness: AppHarness, mint_token):
    response = _call(
        ...
        {"tenant_slug": TENANT, "operation_id": OP_A1, "type": "docx", "s3_key": f"approved_pdfs/{OP_A1}/v3_doc.docx"},
    )
    assert "invalid_arguments" in _tool_error_text(response)
    ...
    second = tool_payload(
        _call(
            ...
            "solstice_commit_operation_version",
            {
                ...
                "type": "html",
                "s3_key": prep2["s3_key"],
                "base_message_id": first["head_message_id"],
            },
        )
    )
...
def test_two_prepares_never_share_a_key(app_harness: AppHarness, mint_token, kind):
    ...
    a = tool_payload(
        _call(
            ...
            token,
            "solstice_prepare_operation_version",
            {"tenant_slug": TENANT, "operation_id": OP_A1, "type": kind, "file_name": f"same.{kind}"},
        )
    )
    b = tool_payload(
        _call(
            ...
            },  # identical name, no commit in between
        )
    )
...
def test_prepared_key_survives_a_version_landing_first(app_harness: AppHarness, mint_token):
    ...
    ok = tool_payload(
        _call(
            ...
            "solstice_commit_operation_version",
            {
                ...
                "type": "html",
                "s3_key": slow["s3_key"],
                "base_message_id": landed["head_message_id"],
            },
        )
    )
...
def test_commit_rejects_key_with_wrong_version_segment(app_harness: AppHarness, mint_token):
    ...
    bogus = (
        "cg_operation_msg_html/00000000-0000-0000-0000-000000000401/v99/00000000-0000-0000-0000-000000000099/v99.html"
    )
    response = _call(
        app_harness,
        token,
        "solstice_commit_operation_version",
        {"tenant_slug": TENANT, "operation_id": OP_A1, "type": "html", "s3_key": bogus, "base_message_id": MEMBER_BASE},
    )
    assert "invalid_key" in _tool_error_text(response)
...
def test_commit_rejects_wrong_prefix(app_harness: AppHarness, mint_token):
    ...
    response = _call(
        ...
        token,
        "solstice_commit_operation_version",
        {"tenant_slug": TENANT, "operation_id": OP_A1, "type": "html", "s3_key": other, "base_message_id": MEMBER_BASE},
    )
    assert "invalid_key" in _tool_error_text(response)
...
def test_commit_rejects_a_base_that_is_no_longer_the_head(app_harness: AppHarness, mint_token):
...
def test_commit_conflicts_when_a_version_lands_mid_flight(app_harness: AppHarness, mint_token):
...
def test_commit_rejects_a_base_on_an_operation_with_no_documents(app_harness: AppHarness, mint_token):
    token = mint_token(sub=SHARED_SUB)
    prep = _prepare_and_upload(app_harness, token, OP_A2, "html", None)
    response = _call(
        app_harness,
    ...
    payload = tool_payload(
        _call(
            ...
            "solstice_commit_operation_version",
            {
                ...
                "type": "html",
                "s3_key": prep["s3_key"],
                "base_message_id": "00000000-0000-0000-0000-000000000503",
            },  # m3's row id
        )
    )
    assert payload["intent"] == "draft"
...
def test_commit_refused_when_every_version_is_hidden(app_harness: AppHarness, mint_token):
    ...
    tool_payload(
        _call(
            ...
            staff,
            "solstice_commit_operation_version",
            {"tenant_slug": TENANT, "operation_id": OP_A2, "type": "html", "s3_key": prep_staff["s3_key"]},
        )
    )
...
def test_commit_needs_no_confirmation_when_visible_head_is_final(app_harness: AppHarness, mint_token):
...
def test_commit_reports_upload_and_key_errors_before_asking_to_confirm(app_harness: AppHarness, mint_token):
