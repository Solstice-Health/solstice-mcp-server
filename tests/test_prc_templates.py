...
def test_prc_template_rules_comes_from_the_backend_unchanged(
    ...
    # `enforcement` is why this endpoint exists rather than a shipped file: it
    # says which rules reject a save, which no document alone can know.
    assert {
        rule["enforcement"] for bucket in payload["rules"].values() for rule in bucket
    } <= {"backend", "engine", "advisory"}
