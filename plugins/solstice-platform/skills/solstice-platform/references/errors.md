...
- **`conflict: not_latest_document`:** a new version landed while you were working, so nothing was written. Re-read the review, reapply the change on top of the new `head_message_id` (row `id`), and commit that. Never re-send the stale `base_message_id`.
...
- **`invalid_request: base_message_id is required`:** the read was skipped. Call `solstice_operation_messages`, take `head_message_id`, and commit with it — omit it only when that value is null.
