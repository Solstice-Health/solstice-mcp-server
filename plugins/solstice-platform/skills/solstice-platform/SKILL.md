...
- **Memory and recent work:** use `solstice_list_recent_work` for recently opened projects/operations and `solstice_memory_recall` for brand, brand-personal, or tenant-personal facts. Bounded non-memory tool activity is observed automatically. The MCP cannot read host conversation or guarantee a literal last call, so the host must call `solstice_memory_observe` once when a durable preference, convention, or decision is observed. Send only a bounded semantic summary and canonical ID refs—never document bodies, claims, prompts, copied content, arbitrary results, or credentials. Scope is caller intent; Backend is authoritative, and brand candidates require approval. Remember, replace, or forget only on an explicit user request via `solstice_memory_remember`, `solstice_memory_replace`, `solstice_memory_forget`. Personal writes need MEMBER; brand writes need ADMIN or SOLSTICE_STAFF. See [Memory policy and safe wording](references/memory.md).
