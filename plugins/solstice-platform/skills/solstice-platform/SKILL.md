...
- **Memory:** recall brand, brand-personal, or tenant-personal facts with `solstice_memory_recall`. When a durable personal preference or convention is observed, call `solstice_memory_observe` once as a cooperative host finalizer; never imply the observation became active memory. Automatic brand memory is unsupported. Remember, replace, or forget a fact only on an explicit user request via `solstice_memory_remember`, `solstice_memory_replace`, or `solstice_memory_forget`. See [Memory policy and safe wording](references/memory.md).
