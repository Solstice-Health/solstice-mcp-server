...
- **Open/read document:** call `solstice_operation_html` and return the time-limited `url` (creative) and `prc_proof_url` (bake, when present). To read, summarize, save, visualize, or convert the body, GET those URLs — the tool does not inline HTML.
...
- **Staff: create a PRC template version:** after the user approves the final HTML preview, ask whether to bake it onto an associated operation (new draft version + `prc_template_s3_key`), publish to the library (`prc_template_versions`), or both. Library / both: ask separately for the template name and template key, then call `solstice_create_prc_template_version` with `html_template` and that `publish_target`. It defaults to published, rejects reserved auto-resolving keys, and never changes brand or operation catalog selections. Operation / both: pass `operation_id` plus the approved Contract v2 bake as `operation_bake_html` when it fits the inline cap, or call `solstice_prepare_prc_template_bake`, PUT to `upload_url`, then pass `operation_bake_s3_key`; a raw catalog shell is not a bake. Use the `prc-template-recreation` skill for the standalone repair and validation workflow.
