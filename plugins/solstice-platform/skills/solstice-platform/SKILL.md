...
- **Staff: create a PRC template version:** after the user approves the final HTML preview, ask whether to bake it onto an associated operation (new draft version + `prc_template_s3_key`), publish to the library (`prc_template_versions`), or both. Library / both: ask separately for the template name and template key, then call `solstice_create_prc_template_version` with that `publish_target`. It defaults to published, rejects reserved auto-resolving keys, and never changes brand or operation catalog selections. Operation / both: pass `operation_id`. Use the `prc-template-recreation` skill for the full workflow.
