...
- **Memory and recent work:** use `solstice_list_recent_work` for recently opened projects/operations and `solstice_memory_recall` for explicit brand, brand-personal, or tenant-personal facts. Bounded non-memory tool activity is observed automatically, but the MCP has no host conversation or turn boundary: never infer preferences, conventions, or brand decisions. Remember, replace, or forget only on an explicit user request via `solstice_memory_remember`, `solstice_memory_replace`, `solstice_memory_forget`. Personal writes need MEMBER; brand writes need ADMIN or SOLSTICE_STAFF. See [Memory policy and safe wording](references/memory.md).
