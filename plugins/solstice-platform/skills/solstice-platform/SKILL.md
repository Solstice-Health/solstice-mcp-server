...
- **Memory:** recall explicit brand, brand-personal, or tenant-personal memory facts with `solstice_memory_recall`. Remember, replace, or forget a fact only on an explicit user request via `solstice_memory_remember`, `solstice_memory_replace`, `solstice_memory_forget`. Personal writes need MEMBER; brand writes need ADMIN or SOLSTICE_STAFF. See [Memory policy and safe wording](references/memory.md).
