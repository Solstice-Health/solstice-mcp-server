...
- **Staff: create a PRC template version:** after the user approves the final HTML preview, ask separately for the template name and template key, then append it with `solstice_create_prc_template_version`. It defaults to published, rejects reserved auto-resolving keys, and never changes brand or operation selections; use the `prc-template-recreation` skill for the full workflow.
