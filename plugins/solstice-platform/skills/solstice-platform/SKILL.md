...
- **Add document version:** on an explicit request, append a new HTML or PDF version to the selected review using the prepare, upload, and commit sequence. An HTML commit includes its PRC proof when PRC is enabled; do not bake it again. For edit assets, a design source file can be attached with `type="source"`.
- **Staff: create a PRC template version:** after the user approves the final HTML preview, ask whether to apply the proof to an associated operation, publish it to the library, or both. Library / both: ask separately for the template name and template key, then call `solstice_create_prc_template_version` with `html_template` and that `publish_target`. Operation / both: call `solstice_prepare_prc_template_bake`, PUT the approved proof to `upload_url`, then pass `operation_id` and `operation_bake_s3_key`. Use the `prc-template-recreation` skill for repair and validation.
