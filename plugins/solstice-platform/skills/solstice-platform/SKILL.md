...
- **Brand context:** on request, load brand rules (`solstice_brand_rules`), design assets (`solstice_brand_design_assets`), and clinical claims (`solstice_brand_claims`). Hosted proof fonts: keep bake `@font-face`, then brand `font_rules`, then `solstice_list_public_fonts` — design assets are images, not fonts. For Figma → HTML conversion with human-in-loop landing, use the `figma-to-solstice` skill. To replace the ISI across one or more assets, use the `isi-replacement` skill.
...
- **Review activity:** show a bounded summary of the selected review's conversation and document versions. User-facing version labels are `V{display_version}` on html/pdf rows (matches the Solstice stepper); identity for reads and commits is `head_message_id` (row `id`). Never parse `v{n}` from S3 keys.
- **Open/read document:** call `solstice_operation_html` with the selected message row's `id` (`head_message_id` for the current version) and return the time-limited `url` (creative) and `prc_proof_url` (bake, when present). To read, summarize, save, visualize, or convert the body, GET those URLs — the tool does not inline HTML.
...
- **Staff: create a PRC template version:** after the user approves the final HTML preview, ask whether to bake it onto an associated operation (new draft version + `prc_template_s3_key`), publish to the library (`prc_template_versions`), or both. Library / both: ask separately for the template name and template key, then call `solstice_create_prc_template_version` with `html_template` and that `publish_target`. It defaults to published, rejects reserved auto-resolving keys, and never changes brand or operation catalog selections. Operation / both: read `solstice_operation_messages` before composing and keep `head_message_id`; call `solstice_prepare_prc_template_bake`, PUT the approved Contract v2 bake to `upload_url`, then pass `operation_id`, `operation_bake_s3_key`, that row ID as `base_message_id`, and `confirmed=true` — never inline bake HTML. On `conflict: not_latest_document`, re-read, rebuild against the new head, prepare/upload again, and retry with the new ID. A raw catalog shell is not a bake. Use the `prc-template-recreation` skill for the standalone repair and validation workflow.
