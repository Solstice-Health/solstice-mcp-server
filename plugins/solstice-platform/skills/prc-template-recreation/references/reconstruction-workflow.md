...
4. If the source is not from the current brand and steps 2–3 missed, load
   `solstice_brand_rules` for another tenant brand that hosts that family.
5. Fontsource only for a real slug of that family.
6. Stop if still missing; name the family.

Do not stand in a different family.

Images (logos, product shots, textures) follow the same fallback: current
`solstice_brand_design_assets`, then public `permanent_assets/`, then another brand's design assets
when the reference is not from the current brand.
Never copy another brand's `solstice_brand_claims`.
...
After the composed preview exists, inspect it again against the source design
— not the structural exemplar. Do this before showing publish questions.

1. Rasterize every composed page at the source page's native aspect and
   dimensions.
2. Place each screenshot next to the matching PDF page or Figma frame
   screenshot.
3. Check page structure, spacing, type, colors, corners, labels, images, and
   per-frame stills.
4. Any mismatch returns to section 6; rebuild the preview and inspect again.
5. Only then show the user the preview and remaining named deltas.

This is a repair loop, not a one-shot validator. Any contract, hydration,
geometry, export, or visual-match failure returns the agent to section 6 to
modify the local artifact and rerun standalone checks. Do not send a raw shell
or known-invalid bake to `solstice_create_prc_template_version`, and do not
ask the MCP server to compose or repair it.
...
- visual inspect against the source (match or named deltas);
- validation failures or uncertain mappings.

Do not write to Solstice until the user approves. Once conversion, preview, and
the visual inspect are complete, ask these as separate yes/no questions:
