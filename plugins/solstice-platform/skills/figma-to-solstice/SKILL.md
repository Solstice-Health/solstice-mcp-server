...
2. **Context** — resolve workspace/brand, then call `solstice_brand_rules`, `solstice_brand_design_assets`, `solstice_brand_claims`. Fetch one existing final asset of the same content type via `solstice_operation_html` with its message row `id` (GET `url`) as the format exemplar.
