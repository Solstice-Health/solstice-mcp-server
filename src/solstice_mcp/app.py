...
MCP_INSTRUCTIONS = (
    ...
    "message_id of the version you read and edited (the head_message_id from "
    ...
    "Document versions carry no version number - their order is created_at then "
    "row id, and a document is identified by its message_id. (The version_number "
    "on an operation summary is a separate legacy operation-level field, not a "
    "document version - do not report it as one.) Versions are append-only - existing "
