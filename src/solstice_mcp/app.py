...
MCP_INSTRUCTIONS = (
    ...
    "nor a turn-complete boundary and cannot guarantee a literal last tool call. The "
    "host must call solstice_memory_observe once when a durable preference, convention, "
    "or decision is observed. This is a cooperative finalizer, not automatic access to "
    "the conversation. Scope is caller intent; Backend remains authoritative for actor, "
    "scope, reference ownership, classification, and activation. Brand candidates "
    "always require approval. Observations must be bounded semantic summaries with "
    "canonical ID-only references. Never send document bodies, claims, prompts, copied "
    "content, arbitrary tool results, credentials, user_id, or role. "
    "solstice_memory_remember, solstice_memory_replace, and solstice_memory_forget "
    "remain explicit-only writes. "
